...
1. **Input Processing**:
   ...
   - Apply hex encoding for special characters (`&` → `26x` in token parameters)
...
- **Solution**: Hex encoding (`&` → `26x`) before compilation
...
2. Apply hex encoding: `sm_send_token_arg <L&>` → `sm_send_token_arg <L26x>`
